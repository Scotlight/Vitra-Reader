...
├──────────────────────────────────────────────────────────┤
│                 渲染适配层 (Adapter)                      │
│  ContentProvider 接口 ← BookContentAdapter / PdfProvider │
├──────────────────────────────────────────────────────────┤
│                 渲染管线层 (Pipeline)                      │
...
├──────────────────────────────────────────────────────────┤
│                 缓存层 (Cache)                             │
│  BookCache (IndexedDB gzip) + SectionManager   │
...
- `BookContentAdapter` (`contentAdapter.ts:39`) — 所有其他格式的通用适配器
...
每个章节的渲染都经过严格有序的五阶段流水线，由 `renderStageTrace.ts` 追踪和校验：

```typescript
// renderStageTrace.ts:3-9 — 五阶段定义
...
```typescript
export async function runRenderStage<T>(trace, stage, task): Promise<T> {
...
const renderContext = await runRenderStage(trace, 'render', async () => {
...
### 4.2 SectionManager — LRU 淘汰器

**源文件**：`src/engine/cache/sectionManager.ts`

```typescript
// sectionManager.ts:38-76 — 核心缓存逻辑
export class SectionManager {
    private loaded = new Map<string | number, LoadedEntry>()
    private maxLoaded: number  // 默认 5，BookContentAdapter 中设为 10
...
**源文件**：`src/engine/cache/bookCache.ts`
...
1. **SectionManager** (`sectionManager.ts:137-143`)：LRU 淘汰时自动 `revokeObjectURL`
...
| `src/engine/render/renderStageTrace.ts` | 五阶段管线追踪 | 3-9 (阶段定义), 57-66 (阶段校验), 82-99 (计时执行), 121-126 (日志格式化) |
...
| 文件 | 职责 | 关键行号 |
|------|------|---------|
| `src/engine/cache/sectionManager.ts` | LRU 内存管理 | 38-76 (load + 淘汰), 116-143 (evict + release) |
| `src/engine/cache/bookCache.ts` | IndexedDB 持久缓存 | 43-51 (SHA-256 hash), 57-71 (gzip 编解码), 90-112 (读), 117-129 (写) |
...
| `src/engine/pipeline/contentAdapter.ts` | 格式适配器 | 39-56 (构造), 60-81 (缓存预热), 128-152 (章节加载) |
...
### 9.1 统一管线入口：BookPipeline

**源文件**：`src/engine/pipeline/pipeline.ts`
...
`BookPipeline.open()` (行 61-76) 是所有格式的唯一入口：
...
工厂函数根据格式创建对应的 ContentProvider（直连 Provider 或经 BookPipeline 走 Adapter）。
...
- 通过 `paginator.ts` 的 `buildPageBoundariesFromDomIdle` 计算页边界
- 支持键盘翻页（← →）、触摸滑动、鼠标滚轮
...
**源文件**：`src/engine/render/vectorPlanner.ts`

`buildVectorRenderPlan()` 决策函数：
...
**源文件**：`src/engine/render/position.ts`
...
### 19.1 ParsedBook 统一模型

**源文件**：`src/engine/types/book.ts`

```typescript
interface ParsedBook {
    format: EngineBookFormat
    metadata: ParsedBookMetadata
    toc: readonly BookTocItem[]
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `pipeline.ts` | 213 | 统一 open 入口 + 预览预热 |
| `contentAdapter.ts` | 223 | ParsedBook → ContentProvider 适配 |

### engine/render/ (11 文件)
| 文件 | 行数 | 职责 |
|------|------|------|
| `renderStageTrace.ts` | 127 | 五阶段管线追踪 |
| `chapterPreprocessCore.ts` | 253 | Worker 预处理核心 |
| `chapterPreprocessService.ts` | 143 | Worker 服务层 |
| `paginator.ts` | ~200 | 分页引擎 |
| `measure.ts` | ~150 | 离屏 DOM 测量 |
| `canvasMeasure.ts` | ~200 | Canvas 快速测量 |
| `vectorPlanner.ts` | ~80 | 向量化渲染规划 |
| `position.ts` | ~120 | 位置序列化 |
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `sectionManager.ts` | 145 | LRU 内存管理 |
| `bookCache.ts` | 159 | IndexedDB gzip 持久缓存 |
