...
- `translate:config`：翻译配置。API key 字段经过 `safeStorage` 加解密；旧键 `translateConfig` 只作为迁移和同步过滤兼容键保留。
...
翻译配置主键是 `translate:config`，存于 `db.settings`。旧键 `translateConfig` 会在加载时迁移到新键，并继续作为敏感键过滤。
...
- `translate:config` 和旧键 `translateConfig` 不参与 WebDAV 同步。
- API key 不应明文持久化。
- 翻译结果缓存不写入 `settings`。
...
`BookCache` 把可缓存格式的 sections HTML 压缩后写入 `db.settings`，键前缀为 `vcache-`。
...
`SectionManager` 限制同时保留的 section 数量。淘汰时需要执行 `revokeObjectURL` 与 `section.unload()`；`destroy()` 必须释放全部已加载条目。

### 7.3 Adapter 内缓存（L1）

`BookContentAdapter` 是章节 HTML 缓存、section manager 和资源会话的汇合点：
...
- 修改 `storageService.ts`、`syncStorePayload.ts`、`useSyncStore.ts`、`useSettingsStore.ts`、`translateService.ts`、`bookCache.ts`、`assetLoader.ts` 任一文件时，都应回看本文档。
...
- `BookCache` / `SectionManager` / `assetLoader` / `searchIndexCache` 的清理链路回归。
