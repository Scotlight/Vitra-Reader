...
3. 对页窗外的段落、标题、列表、图片等元素做水平窗口裁剪：保留布局壳，脱水子 DOM 和媒体资源，回到页窗后恢复。
...
- 分页可按当前页加 overscan 脱水页窗外的可渲染块，减少非当前页的 DOM 子树、媒体资源、绘制和命中测试成本。
...
- 分页模式不卸载可渲染块的布局壳；布局壳用于保持 CSS columns 页码稳定。
- 分页模式不复用连续滚动的 segment DOM pool。
- 单个超大章节进入分页模式时，仍需要完整 DOM 参与 CSS columns 排版。
- 页窗外元素会释放子 DOM 与媒体资源，但仍保留元素壳和原始 HTML 字符串用于恢复。
...
- 页窗脱水只能移除候选元素的子 DOM 与媒体地址，不得删除候选元素本身；恢复时必须补回原始内容、样式和媒体属性。
