...
- `src/engine/render/renderStageTrace.ts`
- `src/engine/render/vectorPlanner.ts`
...
- 管线顺序：`src/engine/render/renderStageTrace.ts`。
- 计划构建：`src/engine/render/vectorPlanner.ts`。
...
- `buildVectorRenderPlan()` 的启用条件和首批段数决策。
