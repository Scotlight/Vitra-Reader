...
import {
    getVectorContentContainer,
    insertVirtualSegmentInOrder,
    updateVirtualContentHeight,
    updateVirtualSegmentLayout,
} from './virtualChapterDom';
...
export function useVirtualChapterRuntime(options: UseVirtualChapterRuntimeOptions) {
    ...
    const mountVirtualSegment = useCallback((runtime: VirtualChapterRuntime, segmentIndex: number) => {
        ...
        segmentEl.style.containIntrinsicSize = `auto 0 ${segment.estimatedHeight}px`;
