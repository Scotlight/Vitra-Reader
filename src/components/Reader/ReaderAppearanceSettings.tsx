...
/**
 * 布局约定（用户定稿）：主题、翻页模式两组常驻顶部一眼可见；
 * 其余全部收进可折叠分区、默认收起——滑条类控件一律不裸露在首屏。
 * readingModeSlot 由面板层注入（翻页模式依赖 bookFormat/onPageTurnModeChange，
 * 不属于本组件的职责面）。
 */
export function ReaderAppearanceSettings({ readingModeSlot }: { readonly readingModeSlot?: ReactNode }) {
    ...
    return (
        <>
            ...
            <ReaderSettingsSection title="颜色">
                <div className={styles.settingsGroup}>
                    <label>文字颜色</label>
            ...
            <ReaderSettingsSection title="状态栏">
                <div className={styles.settingsGroup}>
                    <label>状态栏内容</label>
            ...
            <ReaderSettingsSection title="界面效果">
                <RangeControl label={`背景模糊: ${settings.uiBlurStrength}px`} min={0} max={40} step={1} value={settings.uiBlurStrength} onChange={(value) => settings.updateSetting('uiBlurStrength', value)} />
                <RangeControl label={`面板透明: ${Math.round(settings.uiOpacity * 100)}%`} min={0.5} max={1} step={0.05} value={settings.uiOpacity} onChange={(value) => settings.updateSetting('uiOpacity', value)} />
            </ReaderSettingsSection>
        </>
    )
}
