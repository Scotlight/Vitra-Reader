...
export function ThemeTypographySettingsCard({
    ...
    const showTheme = scope !== 'typography'
    const showTypography = scope !== 'theme'
    const title = scope === 'theme' ? '主题与配色' : scope === 'typography' ? '排版' : '主题与排版'

    return (
        <SettingsCard title={title}>
            {/* 横跨整行后内部两列，避免七项单列把卡片撑得过高 */}
            <div className={styles.themeTypographyGrid}>
                {showTheme && (
                    <div className={styles.themeTypographyThemeRow}>
                        <SettingRow label="主题">
