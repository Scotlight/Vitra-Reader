...
interface TranslateSettingsTabProps {
    scope?: 'all' | 'cache' | 'service'
}

export function TranslateSettingsTab({ scope = 'all' }: TranslateSettingsTabProps) {
    const {
    ...
    const showService = scope !== 'cache'
    const showCache = scope !== 'service'
        ...
        <div className={styles.syncPanel}>
            ...
            <div className={styles.syncActions}>
                <button className={styles.smallBtn} onClick={handleSaveTranslateConfig} disabled={translateSaving}>
                    {translateSaving ? '保存中...' : scope === 'cache' ? '保存缓存配置' : '保存翻译配置'}
                </button>
                {showService && (
