...
describe('translateConfig', () => {
    ...
    it('缺少 safeStorageEncrypt 时保存 API key 原文', async () => {
        ...
        expect(storageMocks.settingsPut).toHaveBeenCalledWith({
            ...
            value: expect.objectContaining({ deeplApiKey: 'plain-key' }),
        })
    })

    it('保存时转换两个 API key 字段且不改非目标字段', async () => {
        const safeStorageEncrypt = vi.fn(async (value: string) => `encrypted:${value}`)
        installElectronApi({ safeStorageEncrypt })
