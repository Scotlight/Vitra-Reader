...
本文件**只承载本项目特有的硬约束**。全局 `~/.claude/CLAUDE.md` 已覆盖的（中文输出、ACE 工具优先级、不主动 push、不主动 commit、Git Safety Protocol、destructive 命令需确认 等）不再重复，默认遵守。

优先级：用户当前会话明确指令 > 本文件 > 全局 `~/.claude/CLAUDE.md` > 项目根 `AGENTS.md`。冲突时**先报告**而不是擅自决定。

---

## 0. `#allow` 直接改代码闸门 [硬性，总闸]

**默认 CC 不直接 Write/Edit 项目源代码**（`src/` / `electron/` 等业务代码）。默认行为是：出 plan / 给建议 / 预览 diff，**不实际写入**。用户在当前消息里显式写 `#allow` 才放行实际写入。

> 本闸门**只约束 CC 自己的手**（CC 直接 Write/Edit）。**与 codex 无关**：codex 走 §6 人工中转流程（用户手动喂 plan 给 codex），codex 改代码**不需要 `#allow`**，CC 也**不得**拿本闸门去拦 codex 产出。CC 对 codex diff 的复查、跑回归、报告 commit 候选，照 §6 原样进行，不受 `#allow` 影响。

- **逐段授权**：一个 `#allow` 只放行**一段独立逻辑改动**（粒度同 §2「一个 commit = 一个独立逻辑改动」）。这一段改完，下一段改动需要**新的** `#allow`。
- **不累积、不延续**：`#allow` 不跨消息、不跨会话累积。当前这段改完即失效，不因为「上一条说过」就默认后续都放行。
- **用户忘说时 CC 必须提醒**：用户要求改代码但当前消息没写 `#allow` → CC **不直接动手**，先出方案/diff 预览，并明确提醒一句「这段需要 `#allow` 才能直接改」。绝不沉默地擅自写入。
- **只放行「写入」这个动作，不解除任何底线**：`#allow` **不**豁免 §1 不可逆禁区、§3 多会话冲突防御（写前 `git status` 必须 clean）、§2 push 前确认的三类例外。这些独立生效，`#allow` 管不到。
- **覆盖 §5 例外 1**：原「用户当前会话明确说『这次你自己改 Reader』」这一例外，统一以 `#allow` 为准——Reader 高敏区直接改同样需要 `#allow`。
- **不受本闸门约束的**：纯文档/规则文件（`.md`）、配置说明、`outputs/runtime/` 工作流副产品的写入，不需要 `#allow`；本闸门只管业务源代码。
...
1. 用户在当前消息显式写 `#allow`（见 §0 总闸）——逐段授权，一个 `#allow` 放行一段独立改动
...
## 6. CC 与 codex 协作 [CC 经 codex-coder 派发]

codex 走 OpenAI 官方 codex 插件，CC **可经 `codex-coder` subagent 主动派发**（该 subagent 是项目硬约束适配层，注入禁区/allowlist/git 规则后转发给官方 `/codex:rescue`）。**不再走"用户手动喂 plan"的人工中转**。CC 的本质不变：**出 plan + 派发 + 复查**——codex 写、CC 审。

> 历史背景：本节原为「人工中转，CC 不调 codex」，写于手搓 wt/spawn 派发路线尚未跑通时。OpenAI 官方插件（`codex-coder` subagent → `/codex:rescue`）已端到端实测可用，故 CC 现在经 `codex-coder` 直接派发，无需用户中转。详见 memory `codex-dispatch-wt-truncation.md`。
>
> ⚠️ **不要裸调 `mcp__codex__codex`**：实测多次空返回、不可靠。正路是 `codex-coder` subagent → 官方 `/codex:rescue`。

底线（不因自动派发而放松）：
- **派发 ≠ 放任**。CC 仍须先出 plan（见步骤 1）、派发后逐文件复查 diff（见步骤 3）、跑回归（步骤 4）、报告候选 commit 等用户确认（步骤 5）。
- **派发前 `git status` 必须 clean**（§3 多会话冲突防御独立生效，§0 `#allow` 管不到）。
- codex 改业务源代码**不需要 `#allow`**（§0 已明确：`#allow` 只约束 CC 自己的手）。

工作流：

1. **CC 出 plan**：列改动文件、敏感符号影响面、新增/调整的 hook 拆分意图、单测要求。plan 落到 `outputs/runtime/<ts>-<topic>.md`
2. **CC 经 `codex-coder` 派发**：把 plan 交给 `codex-coder` subagent 实施（它会前置注入项目禁区/allowlist/git 协作规则再转发官方 `/codex:rescue`）。**判成败看 `/codex:result` 输出 + git diff 是否产生预期改动，绝不看退出码**（codex 答对也常 `exit=1`）
3. **CC 接 diff 复查**：codex 完成后，CC 主动 `git diff` + `git diff --stat` 自审。命中以下任一立即 `git stash push -m "codex-out-of-scope-<ts>"` 留人工：
   - 改动文件超出 plan 范围
   - §3 多会话冲突 6 条信号命中 ≥3
...
> 本节推翻"少写注释"的默认行为。用户审代码时靠注释快速回忆意图，走神一次就忘了这段在干嘛，所以关键路径必须有中文 why 注释。

**必须写注释的场景**（写代码、复查 codex diff 时都适用）：

- 非显然的逻辑：位偏移、二进制格式解析、魔数、编解码、LZ77/回看等算法细节
- 有前提假设的分支：为什么选这个阈值、为什么先 A 后 B、为什么容忍某种畸形输入
- 资源生命周期：Blob URL / 缓存 / AbortController 谁持有、何时释放、为什么此处不能 revoke
- 跨模块契约：调用方必须按某顺序传参才成立的复杂度保证、共享状态的所有权边界

**注释内容规范**：

- 写 **why 不写 what**——解释"为什么这么做 / 这么做的前提是什么"，不复述代码字面在做什么
- 一律**中文**，与代码同处一屏能秒懂
- 关键路径宁可多写一句 why，也不要让审代码的人靠猜

**禁止的注释**（这些才是要避免的）：

- 复述代码的废话：`// 循环 items`、`// 设置 x 为 1`
- 与代码不同步的过期注释——改逻辑必同步改注释
- 被注释掉的死代码（用 git 历史，不留在源码里）

判据：一句注释若删掉后审代码的人会卡壳 → 保留；删掉毫无影响 → 它是废话，不写。
